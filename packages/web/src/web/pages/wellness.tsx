...
function recommendationFor(checkin: any) {
  const allVideos = (classesData as any[]).flatMap((c) => c.videos);
  ...
  const match = allVideos.find((v) => v.style === style) ?? allVideos[0];
  return { match, reason };
}
...
function WellnessContent() {
  ...
  const save = useMutation({
    mutationFn: async () => {
      const res = await api["checkins"].$post({
        json: { mood, energy, sleepRating, cyclePhase, notes },
      });
      return res.json();
    },
    onSuccess: () => {
